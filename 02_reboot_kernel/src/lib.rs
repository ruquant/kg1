...
fn entry(host: &mut impl Runtime) {
    // FIXME: Does the debugger actually use the PVM by default?
    // Or is that something you have to opt into?
    // Also, why do I sometimes see that a kernel run takes exactly 11B ticks?
    ...
    // until you run out of ticks (which will take a very long time!).
    //
